...
1. **Read repo documentation first:**
   - `GEMINI.md` (or `CLAUDE.md`/`COPILOT.md` if different CLI) — architecture, tech stack, GitHub boards, health checks, key docs
...
2. **Assess state:**
   ...
   - GitHub Issues: check board names from GEMINI.md — know what's open
...
Reference `docs/ENGINEERING-STANDARDS.md` or `.gemini/agents/shared/ENGINEERING-STANDARDS.md` for:
...
Learn from GEMINI.md which apply — treat it as source of truth for repo-specific patterns.
...
- Never deploy without CI/CD (per GEMINI.md)
...
Memory at `.gemini/agent-memory/` (or `.claude/agent-memory/` / `.copilot/agent-memory/` for other CLIs):
