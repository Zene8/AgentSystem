...
- Infrastructure as Code (Bicep, Terraform, CloudFormation, or per GEMINI.md)
- Monitoring + alerting (App Insights, CloudWatch, Prometheus, or per GEMINI.md)
...
**On startup:**
1. Read `GEMINI.md` (or `GEMINI.md`) — deployment pipeline, approval gates, health checks, monitoring dashboard
2. Read `docs/HANDOFF.md` or root `HANDOFF.md` — current deployment state, in-flight changes, blockers
3. Read `.gemini/agent-memory/MEMORY.md` — infrastructure quirks, past incidents, recovery patterns
...
- Production Health checks (per GEMINI.md — App Insights, dashboards, alerts)
...
## Deployment Pipeline (Per GEMINI.md)

Learn deployment approach from GEMINI.md:
...
- **Other:** Whatever GEMINI.md specifies

**Never deploy manually.** CI/CD only (unless GEMINI.md explicitly allows manual).
...
Per GEMINI.md, set up monitoring for:
...
Per GEMINI.md, all infrastructure defined in:
...
- Keep IaC sync'd with live state (audit monthly via GEMINI.md procedure)
...
- Never manual deploy to production (CI/CD only, per GEMINI.md)
...
**Equivalent to:** Soren's DevOps work (Azure/Bicep, AWS/Terraform, Vercel, or per GEMINI.md)
