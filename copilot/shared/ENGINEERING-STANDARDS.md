# Engineering Standards — Shared Discipline (Copilot)

All engineering agents (Friday, Ultron, Astra, Pym, Leo) follow these standards. Repo-agnostic: learn specifics from COPILOT.md, GEMINI.md, COPILOT.md, and docs/ folder.

## Startup Protocol (All Engineering Sessions)

1. **Read repo documentation first:**
   - `COPILOT.md` (or `GEMINI.md`/`COPILOT.md` if different CLI) — architecture, tech stack, boards, health checks, deployment patterns
...
2. **Assess current state:**
   ...
   - Check GitHub Issue boards referenced in COPILOT.md (Fax Genie #4, Genie #X, etc.) — know what's open
   - Production health check if COPILOT.md specifies one (App Insights, /api/health, monitoring dashboard)
   - Check `.copilot/agent-memory/MEMORY.md` — persistent engineering patterns + quirks
...
- Tag Issues with agent name + product label (per COPILOT.md convention)
...
### 2. Session Log
- Track in `docs/SESSION-LOG.md` or root `SESSION-LOG.md` (check COPILOT.md for location)
...
- Persistent patterns live in `.copilot/agent-memory/MEMORY.md` (or `.gemini/agent-memory/` for Gemini)
...
- On startup, check production health per COPILOT.md:
...
- When shipping a version (semver bump):
  - Update `CHANGELOG.md` (or `docs/CHANGELOG.md` per COPILOT.md)
  - Format: `## [X.Y.Z] — YYYY-MM-DD` with bullet points per category (Features, Fixes, Security, Breaking Changes, etc.)
  - Reference PR numbers
- Track version in `package.json`, `pyproject.toml`, or equivalent per COPILOT.md
...
- Required technologies (per COPILOT.md)
...
| Need | Source | Pattern |
|------|--------|---------|
| **GitHub boards** | COPILOT.md `## Boards` | Fax Genie #4, Genie #X, etc. |
| **Health checks** | COPILOT.md `## Monitoring` | URL, metrics, dashboard |
| **Tech stack** | COPILOT.md `## Stack` | Python/Azure/Bicep, Node/Vercel, etc. |
| **Deployment** | COPILOT.md `## Deployment` | Manual, CI/CD, approval gates |
| **Key docs** | COPILOT.md `## Key Documents` | Design specs, architecture, runbooks |
| **Compliance** | COPILOT.md `## Compliance` | HIPAA, SOC-2, GDPR gates |
| **Handoff location** | Explicit path in COPILOT.md | `docs/HANDOFF.md` vs root |

**Pattern:** Agents treat COPILOT.md as their north star. Whatever it says is ground truth for that repo.

## Memory Discipline

All agents maintain `.copilot/agent-memory/MEMORY.md` (or `.gemini/` equivalent):
...
- All agents respect repo-specific COPILOT.md (not hardcoded patterns)
...
**Applies to:** All repos with COPILOT.md (Genie, FaxGenie, Basely projects, etc.)
