...
1. **Read repo documentation first:**
   - `COPILOT.md` (or `CLAUDE.md`/`GEMINI.md` if different CLI) — architecture, tech stack, GitHub boards, health checks, key docs
   ...
   - `.copilot/agent-memory/MEMORY.md` — persistent patterns + quirks from prior sessions

2. **Assess state:**
   - `git log --oneline -10` — what shipped since last session
   - GitHub Issues: check board names from COPILOT.md — know what's open
   - Production health per COPILOT.md (App Insights, /api/health, monitoring dashboard)
...
Reference `docs/ENGINEERING-STANDARDS.md` or `.copilot/agents/shared/ENGINEERING-STANDARDS.md` for:
...
- Production Health (COPILOT.md defines specifics)
...
1. **Cross-tenant regression** — does system still work after change?
2. **Contract regression** — do adapters/integrations return expected shape?
3. **Workflow branching** — does edge case take correct path?
...
- What adapter contracts need proofs?
...
Learn from COPILOT.md which apply — treat it as source of truth for repo-specific patterns.
...
2. **Update repo docs:**
   ...
   - `.copilot/agent-memory/MEMORY.md` if new patterns discovered
...
3. **Update Session Log:**
   ...
   - Location: per COPILOT.md (usually `docs/SESSION-LOG.md`)
...
- **Explain the why** — Nathan is building engineering intuition. When something breaks, explain the mechanism.
...
- **Canary tests:** Prove happy path before rollout (every new integration, every new tenant)
...
- Never deploy without CI/CD (per COPILOT.md)
...
Memory at `.copilot/agent-memory/` (or `.claude/agent-memory/` / `.gemini/agent-memory/` for other CLIs):
...
Update index in `.copilot/agent-memory/MEMORY.md` or equivalent.
