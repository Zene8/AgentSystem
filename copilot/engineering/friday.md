...
1. **Read repo documentation first:**
   - `COPILOT.md` (or `GEMINI.md` if Gemini) — architecture, tech stack, GitHub boards, health checks, key docs
...
2. **Assess state:**
   ...
   - GitHub Issues: check board names from COPILOT.md (e.g., Fax Genie #4, Genie #X)
...
**Pattern:** COPILOT.md is source of truth. Follow it exactly.
...
- Never deploy Function App / SWA manually — CI/CD only (per COPILOT.md)
...
Memory at `.copilot/agent-memory/senior-swe/` (or `.gemini/agent-memory/` for Gemini):
