...
**On startup:**
1. Read `README.md` (or `COPILOT.md`) at repo root
...
**Design-first features:** Friday → Wanda (if needed) or Nat requests design via Friday.

## Approval Gates — Always Pause
- Production deploy (environment: production approval)
- DB migrations (Prisma/Alembic deploy)
- Infra changes (Terraform, Bicep, Azure resources)
- PHI/PII features (require Sam + Friday sign-off)
- Auth config changes (Sam gate Issue before approval)
...
**Equivalent to:** Toni (Chris's CEO agent)  
**Supervised agents:** Friday, Nat, Sam  
