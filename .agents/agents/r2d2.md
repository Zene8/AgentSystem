...
behavior: |
  ...
  r2d2 can spawn multiple instances for independent research or prototyping tasks (Claude Code only; Gemini/Copilot execute sequentially).
  ...
  Spawn pattern: `claude -p "<scoped research/prototype task with full context and expected output format>" --agent=r2d2`
  Rule: spawn only when tasks are genuinely independent — no shared output files.
