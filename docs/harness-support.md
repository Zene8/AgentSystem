...
| Secret/PII shield (#222) | `supported` | `gap` | `secret-shield-hook.js` runs on **PostToolUse with no matcher** (redact every tool result) and **PreToolUse** (rehydrate). The bridge maps `PostToolUse`, so the egress half could be bridged; the ingress half cannot, because redaction needs `hookSpecificOutput.updatedToolOutput` to REPLACE a tool result and the bridge has no channel that rewrites one — it can only allow, deny, or inject text. Bridging it partially would be worse than not bridging it: rehydration without the ingress filter puts a real secret into a tool result and is a net loss (see `docs/secret-shield-contract.md`). So in an `agy` session the shield is inert (tracked in #222; general hook gap #240) and `rehydrate` must stay off. |
