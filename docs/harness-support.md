...
| Cost tracking | `supported` | `gap` | `tools/session-cost.js` reads `nexus/session-log.jsonl`, written by `hooks/session-hygiene-hook.js` and `hooks/claude-hooks/session-end.sh`. No hook, no rows — `agy` spend is invisible, not merely unreported. [#240](https://github.com/Zene8/AgentSystem/issues/240). |
