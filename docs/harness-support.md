...
| Continuous sync — memory, session-triggered | `supported` | `gap` | `hooks/continuous-sync-hook.js` pulls `~/agent-memory` at SessionStart and pushes it at SessionEnd. Hook-borne, so an `agy` session neither starts current nor publishes what it learned; its memory writes sit local until some other trigger fires. [#341](https://github.com/Zene8/AgentSystem/issues/341), [#240](https://github.com/Zene8/AgentSystem/issues/240). |
| Continuous sync — code checkout fast-forward | `supported` | `gap` | Same hook, start phase only, via `tools/repo-sync.js` — an `agy` session can therefore run against an AgentSystem checkout that is days behind `main`. `tools/repo-sync.js` is host-neutral and can be run by hand or from any scheduler. [#341](https://github.com/Zene8/AgentSystem/issues/341), [#240](https://github.com/Zene8/AgentSystem/issues/240). |
| Continuous sync — hook redeploy after a pull | `supported` | `gap` | Same hook, start phase only: when `tools/repo-sync.js` reports it pulled, the worker runs `tools/deploy-hooks.js`, so a merged `hooks/**` change reaches `~/.claude` without a human running a command. Hook-borne twice over — `agy` has no hook layer to deploy into and no hook to trigger the deploy — so an `agy`-only host keeps whatever was last installed by hand, and only the daily drift check would notice. [#396](https://github.com/Zene8/AgentSystem/issues/396), [#240](https://github.com/Zene8/AgentSystem/issues/240). |
| Continuous sync — host timer | `supported` | `supported` | `tools/install-brain-sync-timer.sh` (systemd `--user`) and `.ps1` (`schtasks`) run `tools/brain-sync-run.js` every ~15 min outside both harnesses on purpose — the same reason as the Actions watchdog. This is the only sync trigger that reaches a host with no session ever open, which is the host that drifted in [#340](https://github.com/Zene8/AgentSystem/issues/340). |
